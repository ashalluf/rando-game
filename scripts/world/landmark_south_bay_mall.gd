...
##
## Triangle budget, roughly: detailed ~32k, far ~0.7k. The cost is concentrated in the painted
## parking bays (~10k, one MultiMesh), the palms (~7k, two MultiMeshes) and the parking-deck
## columns (~1.5k); everything else is boxes at twelve triangles each. LOT_ROWS and BAY_WIDTH
## move the bay count linearly, PALM_FRONDS moves the palm cost.
...
## Thickness of every paved slab, in metres. Buried under the pad so undulating ground is hidden.
const SLAB_THICK := 1.6
...
static func _site(parent: Node3D, statics: StaticBody3D, base: Vector3, detailed: bool) -> void:
	...
	# The surface car park deck itself: one big slab from the plaza to the south ring leg.
	var lot_depth := SITE_FRONT - 72.0
	_pave(parent, statics, Vector3(LOT_HALF_X * 2.0 + 40.0, SLAB_THICK, lot_depth), Vector3(base.x, base.y + LOT_TOP, base.z + 72.0 + lot_depth * 0.5), asphalt)
	...
	for i in 9:
		Landmarks._box(parent, null, Vector3(0.9, 0.04, 6.0), base + Vector3(-8.0 + i * 2.0, MARK_TOP, 80.0), PAINT_WHITE, false)
...
static func _entrance(parent: Node3D, statics: StaticBody3D, base: Vector3, detailed: bool) -> void:
	...
	letters.mesh = PropFactory.text_mesh("SOUTH BAY MALL", SIGN_LETTER_HEIGHT)
	letters.material_override = PropFactory.material(Color(1.0, 0.93, 0.74), 0.4, true)
	letters.position = base + Vector3(0.0, ENTRY_HEIGHT - 3.0, face + 1.1)
...
static func _pylon_sign(parent: Node3D, statics: StaticBody3D, base: Vector3) -> void:
	var at := base + Vector3(0.0, 0.0, SITE_FRONT - 22.0)
	...
	for dz: float in [-1.0, 1.0]:
		...
		face.mesh = PropFactory.text_mesh("SOUTH BAY MALL", 1.0)
		face.material_override = PropFactory.material(Color(1.0, 0.93, 0.74), 0.4, true)
		face.position = at + Vector3(0.0, 13.6, dz * 1.0)
		face.rotation.y = 0.0 if dz > 0.0 else PI
...
static func _parking_deck(parent: Node3D, statics: StaticBody3D, base: Vector3, batch: MultiMeshBatch, detailed: bool) -> void:
	...
	for level in DECK_LEVELS + 1:
		...
		var slab := Landmarks._box(parent, statics, Vector3(DECK_WIDTH, 0.4, DECK_DEPTH), c + Vector3(0.0, y + 0.2, 0.0), slab_colour, true)
		slab.material_override = concrete
		if level == 0:
			continue
		# Spandrel band round the open edge of each deck.
		for dz: float in [-1.0, 1.0]:
			Landmarks._box(parent, statics, Vector3(DECK_WIDTH + 0.8, 1.1, 0.6), c + Vector3(0.0, y + 0.95, dz * DECK_DEPTH * 0.5), Color(0.68, 0.67, 0.65), false)
		for dx: float in [-1.0, 1.0]:
			Landmarks._box(parent, statics, Vector3(0.6, 1.1, DECK_DEPTH + 0.8), c + Vector3(dx * DECK_WIDTH * 0.5, y + 0.95, 0.0), Color(0.68, 0.67, 0.65), false)
	...
	# Ramps in the east bay, one per level change, tilted slabs you can actually drive.
	var ramp_len := 24.0
	var pitch := atan(DECK_FLOOR / ramp_len)
	for level in DECK_LEVELS:
		var rz := (-1.0 if level % 2 == 0 else 1.0) * (DECK_DEPTH * 0.25)
		var dir := 1.0 if level % 2 == 0 else -1.0
		# +0.4 puts the ramp's two ends exactly on the slab tops above and below it.
		var at := c + Vector3(DECK_WIDTH * 0.5 - 7.0, level * DECK_FLOOR + DECK_FLOOR * 0.5 + 0.4, rz)
		var ramp := Landmarks._box(parent, null, Vector3(10.0, 0.35, ramp_len), at, slab_colour, false)
		ramp.rotation.x = -pitch * dir
		ramp.material_override = concrete
		if statics:
			var shape := CollisionShape3D.new()
			var box := BoxShape3D.new()
			box.size = Vector3(10.0, 0.35, ramp_len)
			shape.shape = box
			shape.position = at
...
static func _lot_markings(parent: Node3D, base: Vector3, batch: MultiMeshBatch) -> void:
	...
	# Accessible stalls nearest the entrance, painted blue.
	for i in 6:
		batch.add("sbm_bay_accessible", accessible, Transform3D(Basis(), base + Vector3(-9.0 + i * 3.6, MARK_TOP - 0.01, LOT_START_Z + BAY_DEPTH * 0.5)))


## True where a planted island interrupts the run of stalls.
static func _island_at(x: float) -> bool:
	for k in 5:
		var ix := -LOT_HALF_X + 6.0 + k * (LOT_HALF_X * 2.0 - 12.0) / 4.0
		if absf(x - ix) < 3.2:
			return true
	return false
...
static func _lot_planting(parent: Node3D, statics: StaticBody3D, base: Vector3, batch: MultiMeshBatch, rng: RandomNumberGenerator) -> void:
	# Planted islands inside the lot: a raised kerb, a palm and a couple of shrubs.
	for row in LOT_ROWS:
		var z0 := LOT_START_Z + row * LOT_ROW_PITCH
		for k in 5:
			var ix := -LOT_HALF_X + 6.0 + k * (LOT_HALF_X * 2.0 - 12.0) / 4.0
			var at := base + Vector3(ix, LOT_TOP, z0 + LOT_ROW_PITCH * 0.5)
			var kerb := Landmarks._box(parent, null, Vector3(5.2, 0.34, LOT_ROW_PITCH - 1.4), at + Vector3(0.0, 0.17, 0.0), Color(0.86, 0.85, 0.82), false)
			kerb.material_override = PropFactory.pbr("sidewalk", 2.5, Color(0.94, 0.93, 0.90))
			Landmarks._box(parent, null, Vector3(4.4, 0.24, LOT_ROW_PITCH - 2.4), at + Vector3(0.0, 0.30, 0.0), Color(0.34, 0.30, 0.24), false)
			_palm(batch, at + Vector3(0.0, 0.4, 0.0), rng.randf_range(8.0, 12.5), rng.randf_range(0.0, TAU), rng)
			for s in 2:
				Landmarks._box(parent, null, Vector3(1.6, 1.0, 1.6), at + Vector3(0.0, 0.85, (s * 2.0 - 1.0) * 5.0), Color(0.26, 0.42, 0.24), false)
		# Light poles down the aisle of each row, set midway between the islands.
		for i in 4:
			var lx := -LOT_HALF_X + 6.0 + (float(i) + 0.5) * (LOT_HALF_X * 2.0 - 12.0) / 4.0
			_light_pole(batch, base + Vector3(lx, LOT_TOP, z0 + LOT_ROW_PITCH * 0.5), rng.randf_range(0.0, 0.2), LIGHT_POLE_HEIGHT)
	...
	# Cart corrals scattered through the lot.
	for i in 4:
		var at := base + Vector3(-72.0 + i * 48.0, LOT_TOP, LOT_START_Z + LOT_ROW_PITCH * 1.5)
	...
	if statics:
		# One shape per island so a car clips the kerb; the palms themselves stay soft.
		for row in LOT_ROWS:
			for k in 5:
				var ix := -LOT_HALF_X + 6.0 + k * (LOT_HALF_X * 2.0 - 12.0) / 4.0
				Landmarks._shape(statics, Vector3(5.2, 0.4, LOT_ROW_PITCH - 1.4), base + Vector3(ix, LOT_TOP + 0.2, LOT_START_Z + row * LOT_ROW_PITCH + LOT_ROW_PITCH * 0.5))
