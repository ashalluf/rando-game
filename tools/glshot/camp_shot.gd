...
## height), PEOPLE=0 leaves the people out, KIT=0 leaves the kit out, SEED (shifts the people's
## seeds, so a different set of rigs), POSES (comma list of the poses to show: 0 sit, 1 lie,
## 2 slump; default all), COWER=1 frightens them first (the slumped cower, the rest get up).
...
func _initialize() -> void:
	...
	if OS.get_environment("PEOPLE") != "0":
		...
		# [pose, x, z, lift]: on the mattress under the tarp, against the wall, on the cardboard,
		# two standing slumped on the pavement, one on cardboard at the kerb side.
		...
		for i in people.size():
			...
			ped.setup_sleeper(Rect2(-60.0, -60.0, 120.0, 120.0), 3.0, 7771 + i * 131 + seed_shift, int(spec[0]), at, PI)
			ped.set("lift", float(spec[3]))
			ped.position = Vector3(at.x, float(spec[3]), at.y)
			root.add_child(ped)
	var cam := Camera3D.new()
	cam.fov = 50.0
	cam.position = Vector3(sin(yaw) * dist, height, 1.3 + cos(yaw) * dist)
	root.add_child(cam)
	cam.look_at(Vector3(0.0, 0.55, 1.3), Vector3.UP)
	cam.current = true
	for i in 16:
		await process_frame
